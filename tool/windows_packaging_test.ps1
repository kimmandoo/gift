...
Assert-Contains $setupLauncher 'CreateShellIntegration' 'setup keeps Explorer integration selection in wizard state'
Assert-Contains $setupLauncher 'AddToPath' 'setup keeps user PATH selection in wizard state'
Assert-Contains $setupLauncher 'Software\Classes\Directory\shell\GIFT' 'setup registers the directory shell command'
Assert-Contains $setupLauncher 'Software\Classes\Directory\Background\shell\GIFT' 'setup registers the directory background shell command'
Assert-Contains $setupLauncher 'Add-GiftUserPath' 'setup can add the installed app to the user PATH'
...
Assert-Contains $uninstaller 'Remove-ShellIntegrationIfTarget' 'uninstall only removes matching Explorer integration entries'
Assert-Contains $uninstaller 'Remove-UserPathEntry' 'uninstall removes only the installed path entry'
