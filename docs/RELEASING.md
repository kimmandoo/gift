...
and registers an uninstaller under the current user's Windows uninstall
entries. The portable executable only runs from a temporary extraction
directory and does not install files, create shortcuts, or show an install
wizard. Neither package requires administrator access. Git remains a
